...
import { faBook, faComments, faExternalLinkAlt, faHandsHelping, faRunning } from '@fortawesome/free-solid-svg-icons';
...
const LinkItem = ({ item, level, ...props }) => {
  const type = !item.href || item.href === '#' ? 0 : item.external ? 2 : item.href[0] === '/' ? 1 : 0;
  ...
  return <Element
    ...
    {...props}>
    {level === 1 && idToIcon(item.id) ? <FontAwesomeIcon icon={idToIcon(item.id)} style={{ minWidth: '1.5em', marginRight: '0.4em' }} /> : null}
    <div style={{ flexGrow: '1' }}>{item.title}</div>
    {type === 2 ? <FontAwesomeIcon icon={faExternalLinkAlt} style={{ marginLeft: '0.4em' }} /> : null}
  </Element>;
}
...
const OappPage = () => {
  ...
  return (
    <PageFrame title="Orientierung">
      <Accordion activeKey={getKey(1)} style={{ marginLeft: '-15px', marginRight: '-15px' }} onSelect={(key) => setKey(1, key)}>
        {tree.map(l1 =>
          <div key={l1.id}>
            <Accordion.Toggle as={LinkItem} item={l1} level={1} eventKey={l1.id} />
            <Accordion.Collapse eventKey={l1.id}>
              <Accordion activeKey={getKey(2)} onSelect={(key) => setKey(2, key)}>
                {l1.items.map(l2 =>
                  l2.href ?
                    <LinkItem key={l2.id} item={l2} level={2} /> :
                    <div key={l2.id}>
                      <Accordion.Toggle as={LinkItem} item={l2} level={2} eventKey={l2.id} />
